...
const ProductList = () => {
    ...
    const fetchData = async (accessToken) => {
        try{
            ...
            const [jsonData, jsonDataStatus] = await TarifasPolizaHelper.fetchData(accessToken);
    ...
    const handleEdit = async (id) => {
        ...
        console.log(editMode);
        console.log(`Tarifa POLIZA con ID ${id}, ha sido Editada`)
    };
    ...
    // TOMA EL CAMBIO DE textField "PRIMA   " 
    const handlePrimaChange = (e, id) => {
        const { value } = e.target;

        setRows((prevRows) =>
            prevRows.map((row) =>
                row.id === id ? { ...row, prima: value } : row
            )
        );
    };

    // TOMA EL CAMBIO DE textField "DEMORA"
    const handleDemoraChange = (e, id) => {
        const { value } = e.target;

        setRows((prevRows) =>
            prevRows.map((row) =>
                row.id === id ? { ...row,  demora: value } : row
            )
        );
    };

        // TOMA EL CAMBIO DE textField "IMPUESTOS INTERNOS"
    const handleImpIntChange = (e, id) => {
        const { value } = e.target;

        setRows((prevRows) =>
            prevRows.map((row) =>
                row.id === id ? { ...row,  impint: value } : row
            )
        );
    };
    
    // TOMA EL CAMBIO DE textField "SELLOS"
    const handleSellosChange = (e, id) => {
        const { value } = e.target;

        setRows((prevRows) =>
            prevRows.map((row) =>
                row.id === id ? { ...row,  sellos: value } : row
            )
        );
    };
    ...
    const handleUpdate = async (id) => {
        ...
        await TarifasPolizaHelper.updateDataById(id, registro);
    ...
    // "SUSTITUYE" a XXXadd. El add se hace desde el mismo form.
    ...
    const handleInsert = () => {
        ...
        if(rows[0].description==="!EDIT" || rows[0].description==="")
        {
            return;
        }
        ...
        setEnableEditPK(false);  
        setOneEditing(false);
        TarifasPolizaHelper.createData(rows[0]) // Pasar el nombre del data al manejador handleAdd
          .then((response) => {
    ...
    return (
        <MainCard title="Maestro TARIFAS.POLIZAS" content={false}>
            <CardContent>
                                            ...
                                            editMode[row.id] ? (
                                                <>
                                                    ...
                                                    <TableCell>
                                                        <TextField
                                                            id="outlined-basic1"
                                                            fullWidth label="PRIMA"
                                                            //defaultValue="Ingrese el nombre de fwette"
                                                            //placeholder='Nueva Descrip'
                                                            value={row.prima}
                                                            onChange={(e) => handlePrimaChange(e, row.id)} // evento que detecta cambio en el textfield
                                                        />
                                                    </TableCell>
                                                    ...
                                                    <TableCell>
                                                        <TextField
                                                            id="outlined-basic1"
                                                            fullWidth label="DEMORA"
                                                            //defaultValue="Ingrese el nombre de fwette"
                                                            //placeholder='Nueva Descrip'
                                                            value={row.demora}
                                                            onChange={(e) => handleDemoraChange(e, row.id)} // evento que detecta cambio en el textfield
                                                        />
                                                    </TableCell>
                                                    <TableCell>
                                                        <TextField
                                                            id="outlined-basic1"
                                                            fullWidth label="IMPUESTOS INTERNOS"
                                                            //defaultValue="Ingrese el nombre de fwette"
                                                            //placeholder='Nueva Descrip'
                                                            value={row.impint}
                                                            onChange={(e) => handleImpIntChange(e, row.id)} // evento que detecta cambio en el textfield
                                                        />
                                                    </TableCell>
                                                    <TableCell>
                                                        <TextField
                                                            id="outlined-basic1"
                                                            fullWidth label="SELLOS"
                                                            //defaultValue="Ingrese el nombre de fwette"
                                                            //placeholder='Nueva Descrip'
                                                            value={row.sellos}
                                                            onChange={(e) => handleSellosChange(e, row.id)} // evento que detecta cambio en el textfield
                                                        />
                                                    </TableCell>       
