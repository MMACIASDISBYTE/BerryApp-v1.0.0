...
const ProductList = () => {
    ...
    const fetchData = async (accessToken) => {
        try{
            ...
            const [jsonData, jsonDataStatus] = await TarifasTerminalHelper.fetchData();
    ...
    const handleEdit = async (id) => {
        ...
        console.log(editMode);
        console.log(`Tarifa TERMINAL con ID ${id}, ha sido Editada`)
    };
    ...
    const handleGastoFijoChange = (e, id) => {
        const { value } = e.target;

        setRows((prevRows) =>
            prevRows.map((row) =>
                row.id === id ? { ...row, gastoFijo: value } : row
            )
        );
    };

    // TOMA EL CAMBIO DE textField "TE"
    const handleGastoVariableChange = (e, id) => {
        const { value } = e.target;

        setRows((prevRows) =>
            prevRows.map((row) =>
                row.id === id ? { ...row,  gastoVariable: value } : row
            )
        );
    };
    ...
    const handleUpdate = async (id) => {
        ...
        await TarifasTerminalHelper.updateDataById(id, registro);
    ...
    const handleInsert = () => {
        ...
        setEnableEditPK(false);  
        setOneEditing(false);
        TarifasTerminalHelper.createData(rows[0]) // Pasar el nombre del data al manejador handleAdd
          .then((response) => {
    ...
    return (
        <MainCard title="Maestro TARIFAS.TERMINALES" content={false}>
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    <Grid item xs={12} sm={6}>
                        <TextField
                            ...
                            placeholder="Buscar en TERMINALES"
                            value={search}
                            size="small"
                        />
                    </Grid>
                                                ...
                                                <>
                                                    ...
                                                    <TableCell>
                                                        <TextField
                                                            ...
                                                            value={row.contype}
                                                            onChange={(e) => handleConTypeChange(e, row.id)} // evento que detecta cambio en el textfield
                                                        />
                                                    </TableCell>
                                                    ...
                                                    <TableCell>
                                                        <TextField
                                                            id="outlined-basic1"
                                                            fullWidth label="GASTO FIJO"
                                                            //defaultValue="Ingrese el nombre de fwette"
                                                            //placeholder='Nueva Descrip'
                                                            value={row.gastoFijo}
                                                            onChange={(e) => handleGastoFijoChange(e, row.id)} // evento que detecta cambio en el textfield
                                                        />
                                                    </TableCell>
                                                    <TableCell>
                                                        <TextField
                                                            id="outlined-basic1"
                                                            fullWidth label="GASTO VARIABLE"
                                                            //defaultValue="Ingrese el nombre de fwette"
                                                            //placeholder='Nueva Descrip'
                                                            value={row.gastoVariable}
                                                            onChange={(e) => handleGastoVariableChange(e, row.id)} // evento que detecta cambio en el textfield
                                                        />
                                                    </TableCell>       
