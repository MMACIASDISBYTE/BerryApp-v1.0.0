...
const fetchData = async (accessToken) => {
    try{
        ...
        const [jsonData, jsonDataStatus] = await ConstHelper.fetchData(accessToken);
    ...
    const handleDelete = async (id) => {
        ...
        await ConstHelper.deleteDataByCode(id);
    ...
    const handleEdit = async (texto, id) => {
        ...
        console.log(editMode);
        console.log(`La constante ${texto} ID ${id}, ha sido Editada`)
    };
    ...
    const handleValChange = (e, id) => {
        ...
        setRows((prevRows) =>
            prevRows.map((row) =>
                row.id === id ? { ...row, val: value } : row
            )
        );
    };

    // TOMA EL CAMBIO DE textField "TE"
    const handleDetalleChange = (e, id) => {
        const { value } = e.target;

        setRows((prevRows) =>
            prevRows.map((row) =>
                row.id === id ? { ...row, detalle: value } : row
            )
        );
    };
    ...
    const handleUpdate = async (id) => {
        ...
        await ConstHelper.updateDataById(id,registro);
    ...
    const handleAddRowToTable = ()=>{
        ...
        const newData = {          // No cambiar este Texto.
            id: 0,
            description: "!EDIT",
            val: 0,
            detalle: ""
    ...
    const handleInsert = () => {
        ...
        if(rows[0].description==="" || rows[0].description==="!EDIT")
        {
            return;
        }
        ...
        setEnableEditPK(false);  
        setOneEditing(false);
        ConstHelper.createData(rows[0]) // Pasar el nombre del data al manejador handleAdd
          .then((response) => {
    ...
    return (
        <MainCard title="Maestro CONSTANTES DE CALCULO" content={false}  >
            <CardContent>
                <Grid container justifyContent="space-between" alignItems="center" spacing={2}>
                    <Grid item xs={12} sm={6}>
                        <TextField
                            ...
                            placeholder="Buscar en CONSTANTES"
                            value={search}
                            size="small"
                        />
                    </Grid>
                                                ...
                                                <>
                                                    ...
                                                    <TableCell>
                                                        <TextField
                                                            ...
                                                            //placeholder='Nueva Descrip'
                                                            value={row.val}
                                                            onChange={(e) => handleValChange(e, row.id)} // evento que detecta cambio en el textfield
                                                        />
                                                    </TableCell>
                                                    <TableCell>
                                                        <TextField
                                                            id="outlined-basic1"
                                                            fullWidth label="Detalles"
                                                            //defaultValue="Ingrese el nombre de fwette"
                                                            //placeholder='Nueva Descrip'
                                                            value={row.detalle}
                                                            onChange={(e) => handleDetalleChange(e, row.id)} // evento que detecta cambio en el textfield
                                                        />
                                                    </TableCell>
