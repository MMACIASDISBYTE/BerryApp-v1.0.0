...
const fetchData = async (accessToken) => {
    try{
        ...
        const [jsonData, jsonDataStatus] = await IIBBHelper.fetchData(accessToken);
    ...
    const handleEdit = async (code, id) => {
        ...
        console.log(editMode);
        console.log(`El IIBB ${code}, ID ${id}, ha sido Editado`)
    };
    ...
    const handleCOEFChange = (e, id) => {
        const { value } = e.target;

        setRows((prevRows) =>
            prevRows.map((row) =>
                row.id === id ? { ...row, coef: value } : row
            )
        );
    };

    // TOMA EL CAMBIO DE textField "TE"
    const handleAlicuotaChange = (e, id) => {
        const { value } = e.target;

        setRows((prevRows) =>
            prevRows.map((row) =>
                row.id === id ? { ...row, alicuota: value } : row
            )
        );
    };
    // TOMA EL CAMBIO DE textField "IVA"
    const handleFactorChange = (e, id) => {
        const { value } = e.target;
    ...
    const handleUpdate = async (id) => {
        ...
        await IIBBHelper.updateDataByCode(registro.code, registro);
    ...
    const handleInsert = () => {
        ...
        setEnableEditPK(false);  
        setOneEditing(false);
        IIBBHelper.createData(rows[0]) // Pasar el nombre del data al manejador handleAdd
          .then((response) => {
    ...
    return (
        <MainCard title="Maestro IIBB" content={false}>
            <CardContent>
                                            ...
                                            editMode[row.id] ? (
                                                <>
                                                    ...
                                                    <TableCell>
                                                        <TextField
                                                            id="outlined-basic2"
                                                            fullWidth label="Codigo Provincia"
                                                            value={row.code}
                                                            onChange={(e) => enableEditPK&&handleCodeChange(e, row.id)}
                                                        />
                                                    </TableCell>
                                                    <TableCell>
                                                        <TextField
                                                            id="outlined-basic1"
                                                            fullWidth label="Provincia"
                                                    ...
                                                    <TableCell>
                                                        <TextField
                                                            id="outlined-basic1"
                                                            fullWidth label="Coef"
                                                            //defaultValue="Ingrese el nombre de fwette"
                                                            //placeholder='Nueva Descrip'
                                                            value={row.coef}
                                                            onChange={(e) => handleCOEFChange(e, row.id)} // evento que detecta cambio en el textfield
                                                        />
                                                    </TableCell>
                                                    <TableCell>
                                                        <TextField
                                                            id="outlined-basic1"
                                                            fullWidth label="Alicuota"
                                                            //defaultValue="Ingrese el nombre de fwette"
                                                            //placeholder='Nueva Descrip'
                                                            value={row.alicuota}
                                                            onChange={(e) => handleAlicuotaChange(e, row.id)} // evento que detecta cambio en el textfield
                                                        />
                                                    </TableCell>
                                                    <TableCell>
                                                        <TextField
                                                            id="outlined-basic1"
                                                            fullWidth label="Factor"
                                                            //defaultValue="Ingrese el nombre de fwette"
                                                            //placeholder='Nueva Descrip'
                                                            value={row.factor}
                                                            onChange={(e) => handleFactorChange(e, row.id)} // evento que detecta cambio en el textfield
                                                        />
                                                    </TableCell>
